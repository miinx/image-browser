...
describe WorkBuilder do
  ...
  context "with a valid & complete work in xml" do
    ...
    before(:context) do
      ...
      @work_builder.parse_input(@xml)
    end

    it "creates a work object for each image" do
      expect(@work_builder.works.size).to eq(2)
      expect(@work_builder.works[0]).to be_instance_of(Work)
    end

    it "parses the camera makes" do
      expect(@work_builder.works[0].make).to eq("Canon")
    end

    it "parses the camera models" do
      expect(@work_builder.works[0].model).to eq("Snappy")
    end

    it "parses the thumbnails" do
      expect(@work_builder.works[0].thumb).to eq("http://test.com/small.jpg")
    end

    it "strips whitespace from values" do
      expect(@work_builder.works[1].make).to eq("Fuji")
    end
  ...
  context "with missing or empty values in work data in xml" do
    ...
    it "still creates work object for each image" do
      expect(@work_builder.works.size).to eq(3)
    end

    it "reports nil for missing values" do
      expect(@work_builder.works[0].make).to be(nil)
      expect(@work_builder.works[1].thumb).to be(nil)
      expect(@work_builder.works[1].model).to be(nil)
    end

    it "reports nil for empty xml nodes" do
      expect(@work_builder.works[2].thumb).to be(nil)
      expect(@work_builder.works[2].make).to be(nil)
    end

    it "still stores valid available values" do
      expect(@work_builder.works[0].thumb).to eq("thumb1.jpg")
    end
